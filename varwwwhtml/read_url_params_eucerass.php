<?php
  if (count($_GET) > 0){
    ...
    $euczmin = $_GET['euczmin'] ?? null;
    $euczmax = $_GET['euczmax'] ?? null;
    $lambdanormmin = $_GET['lambdanormmin'] ?? null;
    ...
    $visual_contamination = intval($_GET['visual_contamination'] ?? 0);
    $day_mode = intval($_GET['day_mode'] ?? 0);
    $survey = $_GET['survey'] ?? "LS";
    $members = intval($_GET['members'] ?? 1);
    $xray = intval($_GET['xray'] ?? 0);
    $sz = intval($_GET['sz'] ?? 0);
    $merger = intval($_GET['merger'] ?? 2);
    ...
    $euczmin = $_POST['euczmin'];
    $euczmax = $_POST['euczmax'];
    $lambdanormmin = $_POST['lambdanormmin'];
    ...
    if (!isset($_POST['merger'])) {
        $merger = 2;
    } elseif ($_POST['merger'] === "1") {
        $merger = 1;
    } elseif ($_POST['merger'] === "0") {
        $merger = 0;
    } else {
        $merger = 2;
    }
    ...
    } elseif ($_POST['visual_contamination'] === "2") {
        $visual_contamination = 2;
    } elseif ($_POST['visual_contamination'] === "3") {
        $visual_contamination = 3;
    } else {
        $visual_contamination = 0;
    }
